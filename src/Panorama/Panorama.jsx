...
import { FaSearch, FaTimesCircle, FaTimes, FaCheckCircle, } from "react-icons/fa";
import { useNavigate } from "react-router-dom";
import SolicitudServicio from "../Servicios/SolicitudServicio";
import CedulaServicio from "../Servicios/CedulaServicio";
    ...
    const fetchEvaluaciones = async () => {
      try {
       ...
       setEvaluaciones(adaptadas);
        ...
        // Opciones de filtros
        const estadosUnicos = [
          ...new Set(adaptadas.map((e) => e.estado)),
                ...
                <tbody>
                  {evaluacionesFiltradas.map((e) => (
                    ...
                    >
                      ...
                      <td>{e.dependencia}</td>
  <td>
                  <span className={`estado-badge ${e.estado.toLowerCase().replace(/\s/g, '')}`}>
                    {e.estado}
                  </span>
                </td>
...
/>
                  ...
                  <button
                    ...
                    onClick={() => {
                      import("xlsx-js-style").then((xlsx) => {
                        const hoy = new Date();
                        const fecha = hoy.toLocaleDateString("es-MX").replaceAll("/", "-");
        ...
        <div className="floating-buttons">
          <button onClick={() => scrollToSection(evaluacionesRef)}>📋 Evaluaciones</button>
          <button onClick={() => scrollToSection(cedulasRef)}>📑 Cédulas</button>
          <button onClick={() => scrollToSection(competenciasRef)}>💡 Competencias</button>
        </div>
