...
import { Area, AreaChart, Cell, Legend, Pie, PieChart, ResponsiveContainer, Tooltip, XAxis, YAxis, BarChart, Bar, CartesianGrid } from "recharts";
import Sidebar from "../Componentes/Sidebar";
import CatalogoDependencia from "../Auxiliares/CatalogoDependencia";

import SolicitudServicio from "../Servicios/SolicitudServicio";
...
function Estadisticas() {
  ...
  const [analistaOptions, setAnalistaOptions] = useState([{ value: "Todos", label: "Todos" }]);
  const [analistaFiltro, setAnalistaFiltro] = useState({ value: "Todos", label: "Todos" });
  const [searchTerm, setSearchTerm] = useState("");
  const [dependenciaOptions, setDependenciaOptions] = useState([{ value: "Todos", label: "Todos" }]);

  const { usuario } = useContext(UsuarioContext);

  const [analistaFiltroEval, setAnalistaFiltroEval] = useState({ value: "Todos", label: "Todos los analistas" });
  const [mesFiltroEval, setMesFiltroEval] = useState(null);

  const [solicitudes, setSolicitudes] = useState([]);
  const [solicitudesRaw, setSolicitudesRaw] = useState([]);
  const [loadingSolicitudes, setLoadingSolicitudes] = useState(true);
  const [estadoFiltro, setEstadoFiltro] = useState({ value: "Todos", label: "Todos" });
  ...
  function mapEstado(fk) {
    switch (fk) {
      case 9: return "Pendiente";
      case 10: return "Entregado";
      case 11: return "Notificado";
  ...
  const [graficaSeleccionada, setGraficaSeleccionada] = useState({ value: "todas", label: "Todas las gráficas" });
  ...
  useEffect(() => {
    const fetchSolicitudes = async () => {
      try {
        ...
        const dataSinAnalista = dataSinBolsa.filter(s => s.FKIdAcceso === null);
  ...
  useEffect(() => {
    const fetchSolicitudes = async () => {
      try {
        const solicitudes = await solicitudServicio.obtenerSolicitudes(token);

        const pendientesCount = solicitudes.filter(s => s.FKIdEstadoProcesoContratacion === 9).length;
        const entregadasCount = solicitudes.filter(s => s.FKIdEstadoProcesoContratacion === 10).length;
        const citadasCount = solicitudes.filter(s => s.FKIdEstadoProcesoContratacion === 11).length;
        ...
        setCitado(
          solicitudes.filter((s) => s.FKIdEstadoProcesoContratacion === 1).length
        );
        setEvaluado(
          solicitudes.filter((s) => s.FKIdEstadoProcesoContratacion === 2).length
        );
      setProcesamiento(
  solicitudes.filter((s) =>
    [13, 14, 15].includes(s.FKIdEstadoProcesoContratacion)
        ...
        setRevision(
          solicitudes.filter((s) => s.FKIdEstadoProcesoContratacion === 4).length
        );
        setFirma(
          solicitudes.filter((s) => s.FKIdEstadoProcesoContratacion === 5).length
        );
        setNotificadoProceso(
          solicitudes.filter((s) => s.FKIdEstadoProcesoContratacion === 6).length
        );
        setCancelado(
          solicitudes.filter((s) => s.FKIdEstadoProcesoContratacion === 7).length
        );
        setTerminado(
          solicitudes.filter((s) => s.FKIdEstadoProcesoContratacion === 8).length
        );
        ...
        if (validas.length > 0) {
          ...
          // Fecha más reciente
          const maxFecha = new Date(Math.max(...validas.map(s => parseFecha(s.fechaNotificacion))));
          ...
          for (let i = 3; i >= 0; i--) {
            ...
            meses.push({ mes: `${mes.charAt(0).toUpperCase() + mes.slice(1)} ${year}`, solicitudes: 0 });
          }
  ...
  useEffect(() => {
    const fetchProcesos = async () => {
      try {
        ...
        // 🔹 Traer solicitudes
        const data = await solicitudServicio.obtenerSolicitudes(token);
        setProcesosRaw(data);
        ...
        analistasData.forEach(a => {
          analistasMap[a.idAcceso] = `${a.nombre} ${a.primerApellido} ${a.segundoApellido || ""}`.trim();
        });
        ...
        const dependenciasUnicas = [
          ...
          ...Array.from(new Set(procesosAdaptados.map(p => p.dependencia))).map(d => ({
            value: d,
            label: d
          }))
        ];
        ...
        const analistasUnicos = [
          { value: "Todos", label: "Todos" },
          ...Array.from(new Set(procesosAdaptados.map(p => p.analista))).map(a => ({ value: a, label: a }))
        ];
...
function mapEstado(fk) {
  switch (fk) {
    ...
    case 9: return "Pendiente (cita)";
    case 10: return "Entregado (cita)";
    case 11: return "Citado";
  ...
  const procesosFiltrados = procesos.filter(p => {
    const coincideEstado = !filtrosProcesos.estado || filtrosProcesos.estado.value === p.estado;
    const coincideAnalista = !filtrosProcesos.analista || filtrosProcesos.analista.value === p.analista;
    const coincideDependencia = !filtrosProcesos.dependencia || filtrosProcesos.dependencia.value === p.dependencia;
    const coincideBusqueda =
    ...
    return coincideEstado && coincideAnalista && coincideDependencia && coincideBusqueda;
  });
    ...
    <div className="estadisticas-page">
      ...
      <main className="main-content-estadisticas">
        ...
        <div className="contenido-cedula-inner">
          ...
          {(graficaSeleccionada?.value === "solicitudes" || graficaSeleccionada?.value === "todas") && (
            ...
            <section className="stats-section">
              ...
              <label className="stats-label">
                {pendientes} Pendientes (cita), {entregadas} Entregadas (cita), {notificadas} Citadas
          ...
          {(graficaSeleccionada?.value === "procesos" || graficaSeleccionada?.value === "todas") && (
            <section className="stats-section">
              <h1 className="section-title">⚙️ {totalProcesos} Procesos Activos</h1>
              <label className="stats-label">
          ...
          {(graficaSeleccionada?.value === "mensual" || graficaSeleccionada?.value === "todas") && (
            ...
            <section className="stats-section">
              ...
              <ResponsiveContainer width="100%" height={300}>
                <AreaChart data={dataMensual}>
                  ...
                  <YAxis />
                  <Tooltip />
                  <Area type="monotone" dataKey="solicitudes" stroke="#18529D" fillOpacity={1} fill="url(#colorSolicitudes)" />
                </AreaChart>
              </ResponsiveContainer>
            </section>
          ...
          {(graficaSeleccionada?.value === "porAnalista" || graficaSeleccionada?.value === "todas") && (
            <section className="stats-section">
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                ...
                <div style={{ width: "250px" }}>
                  <Select
                    ...
                    onChange={(value) => setAnalistaFiltro(value || { value: "Todos", label: "Todos" })}
...
{(() => {
 ...
 const contarTexto = (textoEstado) =>
  procesosFiltradosPorAnalista.filter((p) => p.estado === textoEstado).length;
...
const notificado = contarTexto("Notificado");
  ...
  return (
    <>
      {/* Label con conteos */}
      <label className="stats-label">
        {totalProcesos} Procesos — {citado} Citado, {evaluado} Evaluado,{" "}
        {procesamiento} En procesamiento, {revision} En revisión,{" "}
        {firma} En firma, {notificado} Notificado
      </label>
      ...
      <div className="chart-container">
        <ResponsiveContainer width="100%" height={320}>
          ...
          {(graficaSeleccionada?.value === "evaluacion" || graficaSeleccionada?.value === "todas") && (
            ...
            <section className="stats-section">
                  ...
                  <Select
                    ...
                    onChange={(value) =>
                      setAnalistaFiltroEval(
                        value || { value: "Todos", label: "Todos los analistas" }
                      )
                    }
                    placeholder="Selecciona un analista"
                    isClearable
              ...
              {(() => {
                ...
                const dataBarra = Object.entries(resultados).map(([name, value]) => ({
                  name,
                  value,
                }));
                ...
                return (
                  <>
                    <label className="stats-label">
                      Mostrando {totalProcesos} procesos con resultado de evaluación{" "}
                      {mesFiltroEval
            ...
            <div className="filtros-combobox">
              ...
              <div>
                ...
                <Select
                  ...
                  onChange={(value) =>
                    setFiltrosProcesos((prev) => ({ ...prev, dependencia: value }))
                  }
                  isClearable
                />
              </div>
