...
import ClienteAPI from "./ClienteAPI.js";
  ...
  export default class CedulaServicio{
      constructor(){
      ...
      try {
        ...
        console.error("Error en obtenerCompetenciasPorClasificacionCedula:", error);
        throw error;
      }
    }
...
async registrarCedulaResultados(formData, token) {
  ...
  const datosLimpios = this.limpiarDatos(datos);
    ...
    console.log("JSON que se enviará a /cedula:", JSON.stringify(datosLimpios, null, 2));
  return await this.api.request("/cedula", "POST", datosLimpios, token);
}
...
async archivarCedula(idCedula, token) {
  ...
  return await this.api.request(`/cedula/${idCedula}`, "PUT", datosLimpios, token);
}
    ...
    async obtenerDatoInicialesCedula(idProcesoContratacion,token){
      ...
      try {
        ...
        if (response.error) {
          throw new Error("No se pudieron obtener los datos iniciales de la cédula");
        }
    ...
    {
      try {
        const response = await this.api.request("/cedula/obtencionCedulas", "GET",null,token);
...
async obtenerCedulaPorId(FKIdProceso, token) {
  try {
    ...
    console.log("🔍 Respuesta completa del servidor (obtenerCedulaPorId):", response);

    if (!response || response.error) {
      throw new Error(response?.mensaje || "No se pudo obtener la cédula por el ID del proceso.");
    }
...
async obtenerCedulasActivas(token) {
        try {
            ...
            console.log("🔍 Respuesta completa del servidor (obtenerCedulasActivas):", response);
            if (!response || response.error) {
                throw new Error(response?.mensaje || "Error al obtener las cédulas activas");
            }
 ...
 async registrarCedulaExterna(formData, token) {
  ...
  console.log("JSON que se enviará a /cedula/externa:", JSON.stringify(datosLimpios, null, 2));

  try {
    const response = await this.api.request("/cedula/externa", "POST", datosLimpios, token);

    if (!response || response.error) {
      throw new Error(response?.mensaje || "Error al registrar la cédula externa.");
    }
...
async obtenerCedulaExternaPorIdCedula(FKIdCedula, token) {
        try {
            ...
            console.log("🔍 Respuesta del servidor (obtenerCedulaExternaPorIdCedula):", response);

            if (!response || response.error) {
                throw new Error(response?.mensaje || "No se pudo obtener la cédula externa.");
            }
