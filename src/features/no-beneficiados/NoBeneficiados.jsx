...
function NoBeneficiados() {
  ...
  useEffect(() => {
    const cargarDatos = async () => {
      try {
        ...
        const procesos =
          await new NoBeneficiadoService().ObtenerTodosLosNoBeneficiados(token);
        const mapeados = procesos.map((p) => {
          const dep = CatalogoDependencia.obtenerZonaPorIdDependencia(
            p.FKIdDependencia,
          );
  ...
  const regionOptions = [...new Set(candidatos.map((c) => c.region))].map(
    (r) => ({ value: r, label: r }),
  );
  const profesionOptions = [...new Set(candidatos.map((c) => c.profesion))].map(
    (p) => ({ value: p, label: p }),
  );
  const resultadoOptions = [...new Set(candidatos.map((c) => c.resultado))].map(
    (r) => ({ value: r, label: r }),
  );
          ...
          <div className="filtros-combobox">
            <div>
              ...
              <Select
                ...
                onChange={(value) =>
                  setFiltros((prev) => ({ ...prev, region: value }))
                }
            ...
            <div>
              ...
              <Select
                ...
                onChange={(value) =>
                  setFiltros((prev) => ({ ...prev, profesion: value }))
                }
