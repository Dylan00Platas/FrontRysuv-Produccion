...
function Agenda() {
  ...
  const mapColorEstado = (estado) => {
    // TODO-Desarrollo: Significado de 9, 10, 11
    switch (estado) {
      case 9:
        return "#f1c40f";
      case 10:
  ...
  const handleEventDrop = async (info) => {
    ...
    try {
      ...
      const payload = {
        ...
        // Mantiene citaVirtual TRUE/FALSE correctamente
        citaVirtual: eventoOriginal.citaVirtual,
      };
        ...
        prev.map((ev) =>
          ev.id === id
            ? {
                ...ev,
                start: nuevaFecha,
                backgroundColor: eventoOriginal.atendioCita
                  ? "#d11a2a"
                  : mapColorEstado(Number(eventoOriginal.estado)),
                borderColor: eventoOriginal.atendioCita
                  ? "#d11a2a"
                  : mapColorEstado(Number(eventoOriginal.estado)),
              }
            : ev,
        ...
        <div className="modal-overlay" onClick={() => setModalAbierto(false)}>
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            ...
            <Select
              ...
              isDisabled={
                ![9, 10, 11].includes(Number(eventoSeleccionado.estado))
              }
              styles={{
                control: (base) => ({
                  ...base,
                  ...
                  minHeight: "45px",
                  height: "45px",
                  opacity: ![9, 10, 11].includes(
                    Number(eventoSeleccionado.estado),
                  )
                    ? 0.6
                    : 1, // pequeño efecto visual
                }),
              }}
              value={[
