...
import CatalogoCedula from "../Auxiliares/CatalogoCedulas.js";
...
import Constantes from "../Auxiliares/Constantes.js";
...
function CrearCedulaInterna() {
  ...
  useEffect(() => {
   ...
   useEffect(() => {
    if (cedulaFromNav && dependenciasCargadas) {      
      const dep = dependencias.find(d => d.nombre === cedulaFromNav.dependencia);       
      ...
      setFormData((prev) => ({
        ...
        evaluacionConocimientos: cedulaFromNav.resultadoEvaluacionConocimiento || "",
  ...
  useEffect(() => {
    ...
    const valoresPsicometria = competencias.reduce((acc, comp) => {
      const clavePsicometria = Constantes.nombreCompetenciaMap[comp.nombreCompetencia];
  ...
  const servicio = new CedulaServicio();
  try {
    const proceso = await servicio.obtenerDatoInicialesCedula(formData.idProceso, token);
    const dependencia = CatalogoDependencia.obtenerDependenciaPorId(proceso.FKIdDependencia);
    const cedulaMatch = cedulaOptions.find(opt =>
      opt.label.toLowerCase().includes(proceso.funcionDesempeniar?.toLowerCase() || "")
    );
    ...
    if (proceso) {
      ...
      setFormData(prev => ({
        ...
        evaluacionConocimientos: proceso.resultadoEvaluacionConocimiento || "",
        adscripcion: dependencia
          ? { value: dependencia.idDependencia, label: dependencia.nombre, zona: dependencia.zona }
          : null,        
      }));
    }
  } catch (error) {
...
useEffect(() => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.cedulaSeleccionada) {
      setMensaje({ texto: `⚠️ Debes seleccionar un tipo de cédula antes de guardar`, tipo: "error" });
    ...
    try {
      ...
      if (!idCedula) throw new Error("No se recibió el ID de la cédula registrada");
      await servicio.registrarResultado(idCedula, formData, competencias, token);
      const solicitudData = {        
        FKIdDependencia: formData.adscripcion ? formData.adscripcion.value : null,
      ...
      const respProceso = await servicioSolicitud.actualizarProcesoContratacionCedula(formData.idProceso, solicitudData, token);
       if (respProceso && !respProceso.error) {
        
      setMensaje({ texto: "✅  Cédula, resultados y base de datos actualizados correctamente", tipo: "exito" });
       setTimeout(() => {
        setMensaje("");
      }, 3000);  
        
      } else {
     setMensaje({ texto: " ⚠️ Se guardó la cédula y resultados, pero hubo error al actualizar la base de datos", tipo: "exito" });
       setTimeout(() => {
        setMensaje("");
      }, 3000);  
      ...
      console.error("Error registrando cédula:", error);
       setMensaje({ texto: `❌ Error al registrar : ${error.message}`, tipo: "error" }); 
    setTimeout(() => {
        setMensaje("");
      }, 3000);
    }
  ...
  const handleGenerarPDF = async () => {    
    try {
      ...
      form.getTextField("antecedentesFamiliares").setText(formData.antecedentes || "");
      form.getTextField("resultadoWord").setText(formData.word || "");
      form.getTextField("resultadoExcel").setText(formData.excel || "");
      form.getTextField("resultadoOrtografia").setText(formData.ortografia || "");
      form.getTextField("evaluacionConocimientos").setText(formData.evaluacionConocimientos || "");
      form.getTextField("expectativaLaboral").setText(formData.expectativas || "");
      form.getTextField("experienciaRelacionada").setText(formData.experienciaPuesto || "");
      ...
      form.getTextField("analista").setText("Lic. "+usuario.nombre+" "+usuario.primerApellido+" "+usuario.segundoApellido);
      ...
      competencias.slice(0, 11).forEach((item, index) => {
        ...
        const keyPsicometria = Constantes.nombreCompetenciaMap[item.nombreCompetencia];
        ...
        try { form.getTextField(`competencia${i}`).setText(nombreCompetencia); } catch {}
        try { form.getTextField(`perfil${i}`).setText(perfil.toString()); } catch {}
        try { form.getTextField(`psicometria${i}`).setText(valorPsicometria.toString()); } catch {}
      });
      const resultadoCuantitativo = sumaPsicometria/sumaPerfil;
      const resultadoCuantitativoPorcentaje = (resultadoCuantitativo * 100).toFixed(0);
      form.getTextField("resultadoCuantitativo").setText(`${resultadoCuantitativoPorcentaje}%`);
      ...
      <main className="main-content">
        ...
        {showHelp && (
          <div className="modal-overlay" onClick={() => setShowHelp(false)}>
            <div className="modal-content" onClick={(e) => e.stopPropagation()}>
              <h2>Ayuda</h2>
              <p>
                Esta es la ventana de <strong>Cédula Interna</strong>. 
                Aquí podrás llenar los datos generales, competencias y resultados de un candidato.
              </p>
              <p>
                Ingresa el <strong>Identificador del candidato</strong> y presiona el icono de la lupa 🔍 para cargar la información 
                disponible en la base de datos.
              </p>
              <p>
                Al finalizar, puedes <strong>guardar</strong> la cédula o <strong>generar el PDF</strong> con todos los datos capturados.
              </p>
              <p>
                <strong>
                  Nota: Si modificas información cargada automáticamente y presionas “Guardar”, 
                  los cambios se reflejarán también en la base de datos.
                </strong>
              </p>
        ...
        <div className="contenido-cedula-interna-inner">
          <form className="form-grid" onSubmit={handleSubmit}>
            ...
            <div className="form-group">
              <label className="form-label-evaluacion">Fecha de Elaboración</label>
              <input
                type="date"
                className="form-input"
                value={formData.fechaElaboracion}
                onChange={(e) => handleInputChange("fechaElaboracion", e.target.value)}
              />
            </div>
            ...
            <div className="form-group">
              ...
              <Select
                ...
                onChange={(selectedOption) => {
                  ...
                  handleInputChange("region", selectedOption ? selectedOption.zona : '');
                  handleInputChange("IdDependencia", selectedOption ? selectedOption.value : null);
                }}
            ...
            <div className="form-group">
              <label className="form-label-evaluacion">Antecedentes Familia UV</label>
              <input
                type="text"
                className="form-input"
            ...
            <div className="form-group" style={{ gridColumn: "span 3" }}>
              ...
              <Select
                ...
                value={cedulaOptions.find(option => option.value === formData.cedulaSeleccionada) || null}
                onChange={(option) => handleInputChange("cedulaSeleccionada", option ? option.value : "")}
                      ...
                      <tr key={i}>
                        ...
                        <td>
                          <input
                            type="number"
                            className="form-input"
                            value={formData[Constantes.nombreCompetenciaMap[item.nombreCompetencia]] || ""}
                            onChange={(e) => {
                              const key = Constantes.nombreCompetenciaMap[item.nombreCompetencia];                                                            
            ...
            {[
              ...
              { label: "Evaluación de Conocimientos", key: "evaluacionConocimientos" },
            ].map(({ label, key }) => (
            ...
            {[
              { label: "Expectativas laborales y económicas", key: "expectativas" },
              { label: "Experiencia Relacionada al puesto", key: "experienciaPuesto" },
              { label: "Experiencia (Periodo, Funciones, Organización)", key: "experiencia" },
              { label: "Conclusiones", key: "conclusiones" },
              { label: "Resultados", key: "resultados" },
            ].map(({ label, key }) => (
              <div key={key} className="form-group" style={{ gridColumn: "span 3" }}>
                <label className="form-label-evaluacion">{label}</label>
                <textarea
