...
function Procesos() {
  ...
  const [estadoOptions, setEstadoOptions] = useState([{ value: "Todos", label: "Todos" }]);
    ...
    const fetchData = async () => {
      try {
        ...
        const analistasUnicos = [
          ...
          ...Array.from(new Set(procesosAdaptados.map((p) => p.analista))).map((a) => ({
            value: a,
            label: a,
          })),
        ];
        ...
        const estadosUnicos = [
          { value: "Todos", label: "Todos" },
          ...Array.from(new Set(procesosAdaptados.map((p) => p.estado))).map((e) => ({
            value: e,
            label: e,
          })),
        ];
  ...
  function getStatusByFecha(fechaNotificacion) {
    ...
    if (diffDias <= 15)
      return { color: "red", icon: < FaExclamationTriangle className="alert-icon red" title="Faltan menos de 15 días" /> };
    if (diffDias <= 30)
      return { color: "orange", icon: <FaBell className="alert-icon orange" title="Falta menos de un mes" /> };
            ...
            <table className="tabla-procesos">
              ...
              <tbody>
                {procesosFiltrados.map((p) => {
                  const procesoOriginal = procesosRaw.find((s) => s.idProceso === p.id);
                  const { color, icon } = getStatusByFecha(p.fechaNotificacion);

                  return (
                    <tr key={p.id} onClick={() => navigate("/evaluacion", { state: procesoOriginal })}>
                      <td>{p.folio +" / "+ p.hermesNotificacion}</td>
                      <td style={{ color, fontWeight: color !== "inherit" ? "bold" : "normal" }}>
                      ...
                      <td>{p.analista}</td>
                   <td>
                  <span className={`estado-badge ${p.estado.toLowerCase().replace(/\s/g, '')}`}>
                    {p.estado}
                  </span>
                </td>
                    </tr>
