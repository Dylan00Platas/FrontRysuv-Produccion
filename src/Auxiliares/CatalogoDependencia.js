...
export default class CatalogoDependencia {
  ...
  async cargarDependencias(token) {
    const response = await this.api.request("/catalogo/dependencias", "GET", null, token);
  ...
  static obtenerDependenciaPorNumeroDependencia(numDependencia){
    return CatalogoDependencia.dependencias.find(d => d.numDependencia === numDependencia);
  }
