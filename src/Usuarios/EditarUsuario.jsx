...
function EditarUsuario() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (formData.contrasena && !regexContrasena.test(formData.contrasena)) {
      ...
      setMensaje({
        texto: "⚠️ La contraseña debe tener mínimo 8 caracteres, al menos una mayúscula, un número y un carácter especial",
        tipo: "error"
      });
    ...
    try {
      ...
      setMensaje({ texto: "✅ Usuario modificado correctamente", tipo: "exito" });
      setTimeout(() => {
        navigate("/usuarios");
      }, 1500);
  ...
  const handleDesactivarUsuario = async () => {
    try {
      const token = localStorage.getItem("token");
      const usuarioServicio = new UsuarioServicio();
      const response = await usuarioServicio.desactivarUsuario(formData.idAcceso, token);
      console.log("Usuario desactivado:", response);
      setMensaje({ texto: "🚫 Usuario desactivado correctamente", tipo: "exito" });
        ...
        <form className="form-grid" onSubmit={handleSubmit}>
          ...
          <div className="form-group">
            ...
            <input
              ...
              onInvalid={(e) => {
                e.target.setCustomValidity('El nombre de usuario es obligatorio');
              }}
              required
            />
          </div>
          ...
          <div className="form-group password-group">
            ...
            <div className="password-input-container-usuario">
              ...
              />
              <button type="button" className="password-toggle-usuario" onClick={togglePassword}>
                {showPassword ? <FaEyeSlash /> : <FaEye />}
              </button>
            </div>
            {formData.contrasena && !/^(?=.*[A-Z])(?=.*\d)(?=.*[!@#$%^&*])[A-Za-z\d!@#$%^&*]{8,}$/.test(formData.contrasena) && (
              <span className="error">La contraseña debe tener mínimo 8 caracteres, al menos una mayúscula, un número y un carácter especial</span>
            )}
          </div>
          ...
          <div className="form-group action-buttons">
            <button type="submit" className="btn-crear-usuario">Editar usuario</button>
